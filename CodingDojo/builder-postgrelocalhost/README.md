...
--------------
Этот проект предназначен для того, чтобы тестировать на локали работу с posgreSQL базой данных. Для этого потребуется:
 * установить сервер PostgreSQL
 * создать базу данных как указано в `src\sql\*.sql`
 * скинфигурировать коннекшен к ней в `src\main\resources\com\codenjoy\dojo\server\codenjoy.properties`
...
```
    <properties>
        <codenjoy.version>http://codenjoy.com/codenjoy-contest/board/game/<game_name></codenjoy.version>
    </properties>
    ...
    <dependencies>
        <dependency>
